...
public class GreyJet extends GameObject {
     ...
     *
     * @param gameView The gaming window where the jet will be displayed.
     */
    ...
    public void updatePosition() {
        ...
        if (greyJetMovementPattern.movingRight && !gameObjectHitsLowerBoundary()) {
            position.right(speedInPixel + 3);
        } else if (!greyJetMovementPattern.movingRight && !gameObjectHitsLowerBoundary()) {
            position.left(speedInPixel + 3);
        }
        if (gameObjectHitsRightBoundary() && !gameObjectHitsLowerBoundary()) {
    ...
    public void addToCanvas() {
        if (greyJetMovementPattern.movingRight) {
            gameView.addImageToCanvas("grey_jet.png", position.getX(), position.getY(), 0.80, 0);
        } else {
            gameView.addImageToCanvas("grey_jet_left.png", position.getX(), position.getY(), 0.80, 0);
        }
